...
export const mobileSchema = z.string().trim().min(7).max(40).regex(/^\+?[0-9 ()-]+$/).refine((value) => value.replace(/\D/g,'').length >= 7);
const mobile = mobileSchema;
...
export type ChildrenOptions = { branches: { id: string; code: string; name: string }[]; classrooms: { id: string; branchId: string; name: string; capacity: number; occupancy: number }[]; capabilities: string[]; integration: { finance: { enabled: boolean; implemented: true }; transport: { enabled: boolean; implemented: true }; activities: { enabled: boolean; implemented: true }; availableSteps: ['accounts','children','documents','finance','transport'] } };
