...
export const checkpointStatusSchema = z.object({ id: z.uuid(), label, order: z.number().int().min(0).max(1000), enabled: z.boolean(), meaning: z.enum(progressMeanings), theme: z.enum(statusThemes), outcome: z.enum(['UNRECORDED','PRESENT','ABSENT','AWAITING_RESULT','RESULT_PUBLISHED','NO_EXAM','CHILD_ABSENT','NOT_ASSIGNED','AWAITING_REVIEW','COMPLETED','NOT_COMPLETED','EXCUSED','NO_HOMEWORK']).nullable() }).strict();
...
export const builtInMeanings: Record<string, CheckpointStatus['meaning']> = { UNRECORDED: 'PENDING', PRESENT: 'RESOLVED', ABSENT: 'RESOLVED', AWAITING_RESULT: 'PENDING', RESULT_PUBLISHED: 'RESOLVED', NO_EXAM: 'NOT_APPLICABLE', CHILD_ABSENT: 'NOT_APPLICABLE', NOT_ASSIGNED: 'PENDING', AWAITING_REVIEW: 'PENDING', COMPLETED: 'RESOLVED', NOT_COMPLETED: 'RESOLVED', EXCUSED: 'NOT_APPLICABLE', NO_HOMEWORK: 'NOT_APPLICABLE' };
export const builtInOutcomes = { ATTENDANCE: ['UNRECORDED','PRESENT','ABSENT'], EXAM: ['AWAITING_RESULT','RESULT_PUBLISHED','NO_EXAM','CHILD_ABSENT'], HOMEWORK: ['NOT_ASSIGNED','AWAITING_REVIEW','COMPLETED','NOT_COMPLETED','EXCUSED','NO_HOMEWORK'], STATUS_NOTE: [] };
