...
export const homeworkHistoryQuerySchema = z.object({ overdue: z.enum(['true','false']).default('false'), on: z.iso.date().optional(), from: z.iso.date().optional(), until: z.iso.date().optional(), limit: z.coerce.number().int().min(1).max(100).default(50), offset: z.coerce.number().int().min(0).max(100000).default(0) }).strict().refine((v) => !v.from || !v.until || v.until>=v.from);
