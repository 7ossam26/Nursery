...
describe('Phase 12 parent hub: bilingual scripted DOM, real HTTP/SSE and PostgreSQL',() => {
 ...
 it.each(['en','ar-EG'] as const)('switches children, refreshes live, acknowledges privately and removes disabled/revoked records in %s',async (locale) => {
  ...
  const raw=f.family('PHUBA'); const other=f.family('PHUBB'); raw.children.push(other.children[0]); const a=await f.children.onboard(f.root.token,raw); const unrelated=await f.onboard('PHUBPRIVATE');
  ...
  await screen.findByText('Read page 1',{},{ timeout: 15000 }); const nav=screen.getByRole('navigation',{ name: t('hub.today') }); expect(within(nav).getAllByRole('link')).toHaveLength(5); expect(document.documentElement.dir).toBe(locale==='en' ? 'ltr' : 'rtl'); expect(screen.queryByText('Child PHUBPRIVATE')).toBeNull(); expect(screen.queryByRole('button',{ name: t('homework.publish') })).toBeNull();
  ...
  await user.click(within(nav).getByRole('link',{ name: t('hub.notifications') })); const readButtons=await screen.findAllByRole('button',{ name: t('hub.read') },{ timeout: 15000 }); await user.click(readButtons[0]); await screen.findByRole('button',{ name: t('hub.unread') },{ timeout: 15000 });
  await user.click(screen.getByRole('navigation').querySelector('a[href="/parent/notices"]')!); await screen.findByText('Please bring a notebook tomorrow.',{},{ timeout: 15000 }); await user.click(screen.getByRole('button',{ name: t('hub.acknowledge') })); await screen.findByText(t('hub.acknowledged'),{},{ timeout: 15000 }); expect((await f.app.communication.announcement(parent.token,notice.id)).acknowledged).toBe(true);
  expect((await axe.run(view.container,{ rules: { 'color-contrast': { enabled: false } } })).violations).toEqual([]);
  await user.click(within(screen.getByRole('navigation')).getByRole('link',{ name: t('hub.today') })); await screen.findByText('Read page 2',{},{ timeout: 15000 }); await f.setModule('HOMEWORK',false); await waitFor(() => expect(screen.queryByText('Read page 2')).toBeNull(),{ timeout: 4900 });
  ...
  await f.licensing.blockAccount(f.root.token,a.guardianIds[0],{ reason: 'Explicit admin decision',publicMessage: 'Call nursery office' }); await screen.findByText('Call nursery office',{},{ timeout: 4900 }); expect(screen.queryByRole('navigation')).toBeNull(); expect(screen.queryByText('Read page 2')).toBeNull(); expect(unrelated.childIds).not.toEqual(a.childIds);
 },60000);
