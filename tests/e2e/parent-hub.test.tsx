...
describe('Phase 12 parent hub: bilingual scripted DOM, real HTTP/SSE and PostgreSQL',() => {
 ...
 it.each(['en','ar-EG'] as const)('switches children, refreshes live, acknowledges privately and removes disabled/revoked records in %s',async (locale) => {
  ...
  await screen.findByText('Read page 1',{},{ timeout: 15000 }); const nav=screen.getByRole('navigation',{ name: t('nav.mobile') }); await waitFor(()=>expect(within(nav).getAllByRole('link')).toHaveLength(5)); expect(document.documentElement.dir).toBe(locale==='en' ? 'ltr' : 'rtl'); expect(screen.queryByText('Child PHUBPRIVATE')).toBeNull(); expect(screen.queryByRole('button',{ name: t('homework.publish') })).toBeNull();
