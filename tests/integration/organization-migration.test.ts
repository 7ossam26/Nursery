...
it.each([3,5])('upgrades an actual Phase %i schema once, preserves identity/reservations, and does not reseed edited roles on rerun',async (phase) => {
  ...
  try {
    ...
    for (const name of ['0000_installation_baseline.sql','0001_authentication.sql',...(phase===5 ? ['0002_organization_policy.sql','0003_licensing_settings.sql'] : [])]) {
    ...
    expect((await database.pool.query('select count(*)::int as count from schema_migrations')).rows[0].count).toBe(14);
  } finally {
