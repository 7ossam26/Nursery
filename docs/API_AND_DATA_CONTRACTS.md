...
Phase16 expense/transfer contract: strict schemas in `packages/contracts/src/spending.ts`, immutable sources and deferred source/leg constraints in `0014_spending.sql`, `SpendingService` using the existing FinancialCore/policy/canonical treasury projection. `/api/v1/expenses` implements pending/approval/actual settlement/history and scoped private documents; `/api/v1/finance/transfers` posts paired internal movements. Pending expense creates no cash, settlement is unique, corrected expense totals use the effective replacement amount, and filtered totals exclude transfers. PrivateDocumentStore is shared with child documents; every expense attachment requires fresh scope/capability/module checks. See D42/D44 and [EXPENSES_AND_CLOSING.md](EXPENSES_AND_CLOSING.md).

Phase16 closing contract: migration0015 implements immutable COUNTED/REOPENED successors, source-linked current-date difference adjustments and shared account-lock/SQL protection against changes affecting any active counted date. `ClosingService`, `packages/contracts/src/closing.ts` and `/api/v1/finance/{closings,closing-options,closing-current}` expose count/reopen/adjust and authorized history/current revision. Reopening/adjustment require capability AND sensitive grant AND branch scope; no SYSTEM fallback. Original counts remain immutable; canonical dated expected balance and generated difference use PostgreSQL exact numeric projections. Bilingual page `/administration/closing` separates physical count from explicitly confirmed cash adjustment. D43 and EXPENSES_AND_CLOSING.md record evidence.

Phase16 correction/refund contract: `0016_corrections_refunds.sql`, `packages/contracts/src/corrections.ts`, and `CorrectionService` append typed source records for receipt/expense/transfer reversal or replacement and tuition reduction. Signed allocation reversals, obligation adjustments, credit origins, refunds and every cash leg are immutable and source-linked; deferred PostgreSQL validators require exact legs and prevent negative credit/origin projections. `credit_balances` subtracts both applications and actual refunds. `/api/v1/finance/{correction-options,corrections,refunds}`, source-specific `correct`/`reduce` routes, and `/administration/corrections` expose scoped options and immutable history. FinancialCore idempotency/audit, original/replacement scope, sensitive entitlement, canonical balances, stable lock order and the shared cash-closing guard apply to every path. See D44 and [EXPENSES_AND_CLOSING.md](EXPENSES_AND_CLOSING.md).
...
Phase18 implements `bus_subscriptions`, append-only permission events, activities, selected children, external-consent events and cancellation history in migration0018. Strict schemas and projections live in `packages/contracts/src/transport.ts`; `/api/v1/transport/{options,subscriptions}` and `/api/v1/activities` expose scoped management/history and teacher rosters. BUS/TRIP charges reuse canonical obligations/installments, collections and notifications; zero fees create no receipt. Migration0019 narrowly extends the deferred reduction invariant to target-matched TUITION/TRIP while preserving every other Phase16 correction branch. `CorrectionService.reductionInTransaction` contains the one reduction calculation/write path and lets activity cancellation commit its status/history and source-linked TRIP credits atomically without nested transactions. D46 records eligibility, module-disable, transfer and date defaults.
