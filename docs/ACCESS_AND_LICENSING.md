...
Phase21 adds the delegable `imports.commit` capability. It never grants data access by itself: each import kind additionally requires the interactive capabilities (children.manage+guardians.manage+users.create_parent; billing.manage with FINANCE; payroll.manage+finance.read with FINANCE and PAYROLL, plus users.manage_staff for logins) and the actor's branch/classroom scope for every referenced record. Templates, previews and batch results are private to their creator. Bulk parent/employee logins reserve seats through the same locked allocation as interactive provisioning, so an insufficient quota rolls back the whole batch. Only the SYSTEM actor may assign roles/branches to imported logins. See D49 and [IMPORTS.md](IMPORTS.md).
