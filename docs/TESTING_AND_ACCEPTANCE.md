...
Phase23 A38 evidence: `npm run test:recovery` 3/3 on real PostgreSQL whole-database fixtures covers the prior file-barrier/overlap/redaction/retention/login/file/debt/treasury/settings cases plus populated-target reset, stale-file replacement, a real live CLI restore with automatic healthy `PRE_RESTORE` directory copy and lock, restored worker startup/heartbeat, valid-archive missing/mismatched file rejection, and 0022 archive forward migration to 0023. `tests/integration/support.test.ts` 1/1 adds installation-identity refusal and lock-aware interrupted-run recovery. Bilingual real-HTTP/DOM support 2/2 executes reset, block/unblock, release/restore, staff disable/reactivate and child archival. Full integration passed 32 files/183; units passed 31 files/118; typecheck/lint/build passed. A first full DOM run passed 22 files/59 tests but failed one pre-existing Phase22 Arabic dialog timing assertion; its exact file immediately passed 4/4, the test-only race was fixed, and the final full run passed 23 files/60 tests. `env:check` and an idempotent `release:prepare` were rerun against real targets. Docker build/compose and SIGTERM remain blocked by this Windows host; no live deployment was run. See [DEPLOYMENT_AND_BACKUP.md](DEPLOYMENT_AND_BACKUP.md).

Phase22 A35/A36/A18/A06 evidence: see PROJECT_STATE.md/HANDOFF.md for the exact commands and results (service-worker and theme-contrast units, navigation/shell contracts, bilingual real-HTTP/DOM network-recovery, collections and parent-hub suites with API/download/SSE revocation after a manual block, full DOM regression, workspace lint/type/build). Browser installability, real offline shell and network-tool cache inspection are manual-walkthrough items; no browser automation or live database was used. Details: [PWA_AND_NETWORK.md](PWA_AND_NETWORK.md).
