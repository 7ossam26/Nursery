...
function CountForm({accountId,op}:{accountId:string;op:ReturnType<typeof useFinancialOperation>}) {
 ...
 return <>{current.error&&<p role="alert">{t(current.error)}</p>}<form onSubmit={count}><fieldset className="organization-fields" disabled={op.locked}><legend>{t('closing.record')}</legend><DateField label={t('spending.date')} required value={on} onValueChange={setOn}/><TextField label={t('closing.counted')} required inputMode="decimal" value={counted} onChange={e=>setCounted(e.target.value)}/><TextField label={t('spending.reason')} required maxLength={500} value={reason} onChange={e=>setReason(e.target.value)}/><Button type="submit" disabled={!current.data||current.data.action==='COUNTED'}>{t('closing.record')}</Button></fieldset></form></>;
}
export function ClosingScreen() {
 ...
 return <main className="organization-page"><LanguageSwitcher/><Link to="/account">{t('auth.account')}</Link><Link to="/administration/finance">{t('finance.title')}</Link><h1>{t('closing.title')}</h1><p>{t('closing.help')}</p><OperationNotice op={op}/>{(options.error||history.error)&&<p role="alert">{t(options.error||history.error!)}</p>}
 ...
 {history.data?.map(c=><Card key={c.id} title={`${c.accountCode} — ${formatDateOnly(c.on)}`}><p>{t(`closing.${c.action}`)} — {t('closing.revision')} {c.revision}</p><p>{t('closing.expected')}: {money(c.expected)}</p><p>{t('closing.counted')}: {money(c.counted)}</p><p>{t('closing.difference')}: {money(c.difference)}</p><p>{c.actor} — {c.reason}</p>{c.adjustmentDate&&<p>{t('closing.adjusted')} {formatDateOnly(c.adjustmentDate)}</p>}{c.isCurrent&&c.action==='COUNTED'&&options.data!.canCorrect&&<><Button disabled={op.locked} onClick={()=>{setSelected(c);setAction('reopen');setReason('');}}>{t('closing.reopen')}</Button>{c.difference!=='0'&&!c.adjustmentId&&<Button disabled={op.locked} onClick={()=>{setSelected(c);setAction('adjust');setReason('');setConfirmed(false);}}>{t('closing.adjust')}</Button>}</>}</Card>)}
 <Button disabled={offset===0||op.locked} onClick={()=>setOffset(offset-20)}>{t('hub.previous')}</Button><Button disabled={history.data?.length!==20||op.locked} onClick={()=>setOffset(offset+20)}>{t('hub.next')}</Button>
 {selected&&<form onSubmit={correct}><fieldset className="organization-fields" disabled={op.locked}><legend>{t(action==='reopen'?'closing.reopen':'closing.adjust')} — {selected.accountCode}</legend><p>{formatDateOnly(selected.on)} — {t('closing.difference')}: {money(selected.difference)}</p><TextField label={t('spending.reason')} required maxLength={500} value={reason} onChange={e=>setReason(e.target.value)}/>{action==='adjust'&&<><p>{formatDateOnly(cairoIsoDate())}</p><label><input type="checkbox" checked={confirmed} required onChange={e=>setConfirmed(e.target.checked)}/>{t('closing.confirm')}</label></>}<Button type="submit">{t(action==='reopen'?'closing.reopen':'closing.adjust')}</Button></fieldset></form>}</>}
 </main>;
}
