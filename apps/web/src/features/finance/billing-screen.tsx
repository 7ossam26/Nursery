import { EmptyState, PageHeader , Badge } from '../../components/data-display.js';
...
function Shares({allocations,names}:{allocations:BillingAllocation[];names:Record<string,string>}) {
 return <ul className="record-list">{allocations.map(s=><li key={s.childId}>{names[s.childId]??s.childId}: <span dir="ltr">{money(s.amount)}</span>{s.installments.length>0&&<ul className="record-list">{s.installments.map((i,n)=><li key={n}><span dir="ltr">{formatDateOnly(i.dueOn)} — {money(i.amount)}</span></li>)}</ul>}</li>)}</ul>;
}
function DraftForm({initialChildIds,send}:{initialChildIds:string[];send:Send}) {
 ...
 return <form className="form-stack" onSubmit={submit}><fieldset className="organization-fields"><legend>{t('billing.draft')}</legend><p>{t('billing.help')}</p>
 {(categories.error||choices.error||initial.error)&&<p className="inline-notice inline-notice--danger" role="alert">{t(categories.error||choices.error||initial.error!)}</p>}{error&&<p className="inline-notice inline-notice--danger" role="alert">{t('finance.invalid')}</p>}
 ...
 <div className="action-group"><Button icon="back" variant="secondary" onClick={()=>setOffset(offset-20)} disabled={!offset}>{t('hub.previous')}</Button><Button icon="arrow" variant="secondary" onClick={()=>setOffset(offset+20)} disabled={choices.data?.length!==20}>{t('hub.next')}</Button></div>
 <p>{t('billing.selected')}</p>{children.map(c=><p key={c.id}>{c.code} — {c.name} <Button icon="trash" variant="danger" onClick={()=>setSelected(children.filter(x=>x.id!==c.id))}>{t('billing.remove')}</Button></p>)}
...
function AgreementControls({a,send}:{a:Agreement;send:Send}) {
 ...
 return <>{a.status==='DRAFT'?<Button icon="check" onClick={()=>action('approve')}>{t('billing.approve')}</Button>:a.terms.mode==='MONTHLY'&&<>
 <Button variant="secondary" onClick={async()=>{try {setPreview(await auth.client.business<CatchupPreview>(`billing/agreements/${a.id}/catchup`));} catch(e) {auth.handleError(e);setError(errorKey(e));}}}>{t('billing.preview')}</Button>
 {error&&<p className="inline-notice inline-notice--danger" role="alert">{t(error)}</p>}{preview&&<>{preview.periods.length?<><p>{t('billing.catchupHelp')}</p>{preview.periods.map(p=><div key={p.period}><p dir="ltr">{formatDateOnly(p.period)}</p><Shares allocations={p.allocations} names={a.childNames} /></div>)}<Button onClick={()=>action('catchup',{previewHash:preview.previewHash})}>{t('billing.catchup')}</Button></>:<p>{t('billing.noCatchup')}</p>}</>}
 {!a.endedOn&&<details className="disclosure"><summary>{t('billing.price')} / {t('billing.pause')} / {t('billing.endAction')}</summary><TextField label={t('billing.reason')} required maxLength={500} value={reason} onChange={e=>setReason(e.target.value)} />
 <form className="form-stack" onSubmit={e=>{e.preventDefault();try {action('price',{effectiveFrom:effective+'-01',price:{normalAmount:egpToPiastres(normal),agreedAmount:egpToPiastres(agreed)},reason});} catch {setError('finance.invalid');}}}><TextField label={t('billing.effective')} type="month" required value={effective} onChange={e=>setEffective(e.target.value)} /><TextField label={t('billing.normal')} required value={normal} onChange={e=>setNormal(e.target.value)} /><TextField label={t('billing.agreed')} required value={agreed} onChange={e=>setAgreed(e.target.value)} /><Button type="submit">{t('billing.price')}</Button></form>
 <form className="form-stack" onSubmit={e=>{e.preventDefault();action('pause',{from:from+'-01',until:until+'-01',reason});}}><p>{t('billing.pauseHelp')}</p><TextField label={t('billing.pauseFrom')} type="month" required value={from} onChange={e=>setFrom(e.target.value)} /><TextField label={t('billing.pauseUntil')} type="month" required value={until} onChange={e=>setUntil(e.target.value)} /><Button type="submit">{t('billing.pause')}</Button></form>
 <form className="form-stack" onSubmit={e=>{e.preventDefault();action('end',{endsOn:end,reason});}}><TextField label={t('billing.endDate')} type="date" required min={cairoIsoDate()} value={end} onChange={e=>setEnd(e.target.value)} /><Button variant="danger" type="submit">{t('billing.endAction')}</Button></form></details>}
 </>}</>;
}
export function BillingWorkspace({initialChildIds=[]}:{initialChildIds?:string[]}) {
 ...
 <div className="action-group"><Button icon="back" variant="secondary" disabled={!offset} onClick={()=>setOffset(offset-20)}>{t('hub.previous')}</Button><Button icon="arrow" variant="secondary" disabled={list.data?.length!==20} onClick={()=>setOffset(offset+20)}>{t('hub.next')}</Button><Button icon="refresh" variant="secondary" onClick={()=>list.reload()}>{t('children.refresh')}</Button></div>
 </fieldset></>;
}
export function BillingScreen() {const {t}=useLocale();return <main className="organization-page"><PageHeader title={t('billing.title')} icon="wallet" /><BillingWorkspace /></main>;}
