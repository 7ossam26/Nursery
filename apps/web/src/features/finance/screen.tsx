import { PageHeader, EmptyState } from '../../components/data-display.js';
...
export function TreasuryScreen() {
 ...
 return <main className="organization-page"><PageHeader actions={<><div className="action-group"><Link className="button button--outline" to="/administration/expenses">{t('spending.title')}</Link><Link className="button button--outline" to="/administration/transfers">{t('transfer.title')}</Link><Link className="button button--outline" to="/administration/closing">{t('closing.title')}</Link><Link className="button button--outline" to="/administration/corrections">{t('corrections.title')}</Link></div></>} title={t('finance.title')} icon="wallet" />
 {(options.error||accounts.error)&&<p className="inline-notice inline-notice--danger" role="alert">{t(options.error||accounts.error!)}</p>}{message&&<p className="inline-notice" role={uncertain?'alert':'status'}>{t(message)}</p>}
 ...
 {options.data.canManage&&branchId&&<form className="form-stack" onSubmit={create}><fieldset className="organization-fields" disabled={busy||uncertain}><legend>{t('finance.create')}</legend><p>{t('finance.help')}</p>
 ...
 <TextField label={t('finance.reason')} required maxLength={500} value={reason} onChange={e=>setReason(e.target.value)} /><Button loading={busy} icon="plus" type="submit">{t('finance.create')}</Button></fieldset></form>}
 {accounts.data?.length===0&&<EmptyState title={t('finance.empty')} art="sky" />}{accounts.data?.map(a=><Card key={a.id} title={`${a.code} — ${a.name}`}><p>{t(`finance.${a.type}`)}</p><p dir="ltr">{formatEgp(piastres(BigInt(a.balance)))}</p><p dir="ltr">{formatDateOnly(a.openedOn)}</p>{a.isDefault ? <p>{t('finance.default')}</p>:a.type==='CASH'&&options.data?.canManage&&<Button disabled={busy||uncertain} onClick={()=>makeDefault(a)}>{t('finance.makeDefault')}</Button>}</Card>)}
 <div className="action-group"><Button icon="back" variant="secondary" disabled={!offset} onClick={()=>setOffset(offset-20)}>{t('hub.previous')}</Button><Button icon="arrow" variant="secondary" disabled={accounts.data?.length!==20} onClick={()=>setOffset(offset+20)}>{t('hub.next')}</Button></div></>}
 </main>;
}
