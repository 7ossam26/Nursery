...
export function CollectionsScreen() {
 ...
 {selected.length===1&&<label><input type="checkbox" checked={credit} onChange={e=>{setCredit(e.target.checked);setConfirmed(false);}}/>{t('collections.credit')}</label>}{credit&&<><p>{t('collections.creditHelp')}</p><TextField label={t('collections.creditReason')} required maxLength={500} value={creditReason} onChange={e=>setCreditReason(e.target.value)}/></>}
 ...
 <label><input type="checkbox" required checked={confirmed} onChange={e=>setConfirmed(e.target.checked)}/>{t('collections.confirm')}</label><Button type="submit" disabled={!confirmed}>{t(credit?'collections.recordCredit':'collections.submit')}</Button><Button onClick={()=>{setSelected([]);setConfirmed(false);}}>{t('collections.cancel')}</Button>
 </fieldset></form>}
 {receiptIds.length>0&&<section><h2>{t('collections.receipts')}</h2>{receiptIds.map(id=><ReceiptReference key={id} id={id}/>)}</section>}
...
function FinanceReminderInbox() {const {t}=useLocale();const [offset,setOffset]=useState(0);const notices=useScoped<{id:string;childName:string;categoryName:string;remaining:string;dueOn:string}[]>(`finance/reminders?limit=20&offset=${offset}`);return notices.data?.length?<section><h2>{t('collections.notices')}</h2>{notices.data.map(n=><p key={n.id}>{n.childName} — {n.categoryName} — <bdi dir="ltr">{formatDateOnly(n.dueOn)} · {money(n.remaining)}</bdi></p>)}<Button disabled={!offset} onClick={()=>setOffset(offset-20)}>{t('hub.previous')}</Button><Button disabled={notices.data.length!==20} onClick={()=>setOffset(offset+20)}>{t('hub.next')}</Button></section>:null;}
function ReceiptHistory({branchId}:{branchId:string}) {const {t}=useLocale();const [offset,setOffset]=useState(0);const receipts=useScoped<Receipt[]>(`finance/receipts?limit=20&offset=${offset}${branchId?`&branchId=${branchId}`:''}`);return receipts.data?<section><h2>{t('collections.history')}</h2>{receipts.data.map(receipt=><ReceiptCard key={receipt.id} receipt={receipt}/>)}<Button disabled={!offset} onClick={()=>setOffset(offset-20)}>{t('hub.previous')}</Button><Button disabled={receipts.data.length!==20} onClick={()=>setOffset(offset+20)}>{t('hub.next')}</Button></section>:null;}
