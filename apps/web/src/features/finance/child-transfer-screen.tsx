...
export function ChildTransferScreen() {
 ...
 return <main className="organization-page"><LanguageSwitcher/><Link to="/account">{t('auth.account')}</Link><Link to="/administration/collections">{t('collections.title')}</Link><h1>{t('childTransfer.title')}</h1><p>{t('childTransfer.help')}</p><OperationNotice op={op}/>{(error||options.error||children.error)&&<p role="alert">{t(error||options.error||children.error!)}</p>}
