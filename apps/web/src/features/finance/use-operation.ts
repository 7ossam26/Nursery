...
export function useFinancialOperation(onSaved:()=>void) {
 ...
 function saved() {pending.current=null;setUncertain(false);setCanRetry(false);setMessage('spending.saved');callback.current();}
 async function send() {
  if(!pending.current) return;setBusy(true);setMessage(null);
  try {await auth.client.business(pending.current.path,'POST',pending.current.body);saved();}
 ...
 async function check() {
  ...
  try {const status=await auth.client.business<{status:'COMMITTED'|'NOT_FOUND'}>(`finance/operations/${pending.current.body.operationId}`);if(status.status==='COMMITTED') saved();else {setCanRetry(true);setMessage('finance.notFound');}}
