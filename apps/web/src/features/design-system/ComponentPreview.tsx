...
export function ComponentPreview() {
  ...
  return <AppShell role={role} mode={role === 'support' ? 'support' : 'standard'} pathPrefix="/__preview"><main>
    ...
    <div className="preview-grid">
      ...
      <Card title={t('preview.states')}><div className="state-stack"><Skeleton label={t('state.loading')} /><StatePanel tone="empty" title={t('state.emptyTitle')} body={t('state.emptyBody')} /><StatePanel tone="error" title={t('state.errorTitle')} body={t('state.errorBody')} actionLabel={t('state.retry')} /><EmptyState art="classroom" title={t('preview.emptyTitle')} body={t('preview.emptyBody')} /><div className="button-row"><Button loading>{t('auth.working')}</Button><Button variant="secondary" settled icon="check">{t('form.save')}</Button><Button variant="outline">{t('preview.openModal')}</Button><Button variant="ghost">{t('common.back')}</Button><Button variant="success" icon="check">{t('form.confirm')}</Button><Button variant="danger" icon="trash">{t('form.cancel')}</Button><Button size="compact" variant="quiet">{t('preview.pending')}</Button><Button iconOnly variant="outline" icon="refresh" aria-label={t('state.retry')}><span className="visually-hidden">{t('state.retry')}</span></Button></div><div className="button-row"><Avatar name={samples.childOne} /><Badge>{t('preview.classroom')}</Badge><Badge tone="count">2</Badge><Badge tone="success">{t('preview.present')}</Badge><Badge tone="warning">{t('preview.pending')}</Badge><Badge tone="danger">{t('state.errorTitle')}</Badge><Badge tone="info">{t('preview.exam')}</Badge></div></div></Card>
    </div>
