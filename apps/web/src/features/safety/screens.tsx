import { Badge } from '../../components/data-display.js';
...
function Messages({ error,notice }: { error: MessageKey | null; notice: MessageKey | null }) { const { t } = useLocale(); return <>{error && <p className="inline-notice inline-notice--danger" role="alert">{t(error)}</p>}{notice && <p className="inline-notice" role="status">{t(notice)}</p>}</>; }
...
function GuardianContacts({ guardians }: { guardians: SafetyGuardian[] }) {
  const { t } = useLocale();
  return <><h3>{t('safety.guardians')}</h3><p>{t('safety.whatsappHelp')}</p><ul className="record-list">{guardians.map((g) => <li key={g.id}>{g.fullName} ({g.relationship}) — <a className="button button--outline button--compact" href={`tel:${g.mobile.replace(/[^+0-9]/g,'')}`} dir="ltr">{g.mobile}</a>{' '}
    {g.whatsappNumber ? <a className="button button--outline button--compact" href={whatsAppLink(g.whatsappNumber)} target="_blank" rel="noopener noreferrer">{t('safety.whatsapp')}</a> : <span>{t('safety.noWhatsapp')}</span>}{g.permissions.pickup && <> — {t('children.pickup')}</>}</li>)}</ul></>;
}
function HealthList({ entries,onRetire,busy }: { entries: HealthEntry[]; onRetire?: (entry: HealthEntry) => void; busy: boolean }) {
  ...
  return <ul className="record-list">{entries.map((h) => <li key={h.id}><strong>{t(`safety.${h.kind}`)}{h.severity === 'CRITICAL' && ` — ${t('safety.CRITICAL')}`}:</strong> {h.title}{h.mobile && <> — <a className="button button--outline button--compact" href={`tel:${h.mobile.replace(/[^+0-9]/g,'')}`} dir="ltr">{h.mobile}</a></>}{h.body && <> — {h.body}</>}{!h.active && <> ({t('safety.retired')})</>}
    {onRetire && h.active && <> <Button icon="trash" variant="danger" disabled={busy} onClick={() => onRetire(h)}>{t('safety.retireEntry')}</Button></>}</li>)}</ul>;
}
function HealthForm({ childId,save,busy }: { childId: string; save: Save; busy: boolean }) {
  ...
  return <form className="form-stack" onSubmit={(e) => { void submit(e); }}><fieldset disabled={busy} className="organization-fields"><legend>{t('safety.addEntry')}</legend>
...
function PersonForm({ childId,save,busy,guardian }: { childId: string; save: Save; busy: boolean; guardian: boolean }) {
  ...
  return <form className="form-stack" onSubmit={(e) => { void submit(e); }}><fieldset disabled={busy} className="organization-fields"><legend>{t('safety.addPerson')}</legend>{guardian && <p>{t('safety.pickupGuardianHelp')}</p>}
    ...
    <label className="choice-field"><input type="checkbox" checked={oneDay} onChange={(e) => setOneDay(e.target.checked)} />{t('safety.oneDay')}</label>
...
function ReviewForm({ authorization,save,busy }: { authorization: PickupAuthorization; save: Save; busy: boolean }) {
  ...
  return <fieldset disabled={busy} className="organization-fields"><legend>{t('safety.review')}: {authorization.fullName}</legend>{authorization.review && <p>{t(`safety.${authorization.review.decision}`)}{authorization.review.note && ` — ${authorization.review.note}`}</p>}
    <TextField label={t('safety.reviewNote')} maxLength={500} value={note} onChange={(e) => setNote(e.target.value)} /><div className="action-group"><Button loading={busy} icon="check" onClick={() => { void review('APPROVED'); }}>{t('safety.approve')}</Button> <Button variant="danger" onClick={() => { void review('REJECTED'); }}>{t('safety.reject')}</Button></div></fieldset>;
}
function RestrictionForm({ childId,save,busy }: { childId: string; save: Save; busy: boolean }) {
  ...
  return <form className="form-stack" onSubmit={(e) => { void submit(e); }}><fieldset disabled={busy} className="organization-fields"><legend>{t('safety.addRestriction')}</legend>
...
function ReleaseForm({ view,save,busy }: { view: ChildSafetyView; save: Save; busy: boolean }) {
  ...
  return <form className="form-stack" onSubmit={(e) => { void submit(e); }}><fieldset disabled={busy} className="organization-fields"><legend>{t('safety.record')}</legend><p>{t('safety.recordHelp')}</p>
    ...
    <label className="choice-field"><input type="checkbox" checked={confirmed} onChange={(e) => setConfirmed(e.target.checked)} />{t('safety.callConfirmed')}</label>
    <TextField label={t('safety.note')} maxLength={500} value={note} onChange={(e) => setNote(e.target.value)} />
    <Button loading={busy} icon="check" type="submit">{t('safety.record')}</Button></fieldset></form>;
}
function Records({ records }: { records: PickupRecord[] }) {
  const { t } = useLocale(); return <><h3>{t('safety.records')}</h3>{records.length ? <ul className="record-list">{records.map((r) => <li key={r.id}>{formatDateOnly(r.businessDate)} — {t('safety.collectedBy',{ name: r.collectorName,relationship: r.collectorRelationship,guardian: r.calledGuardianName })}{r.note && ` — ${r.note}`}</li>)}</ul> : <p>{t('safety.none')}</p>}</>;
}
function IncidentList({ incidents,editor }: { incidents: Incident[]; editor?: (incident: Incident) => ReactNode }) {
  ...
  return <ul className="record-list">{incidents.map((i) => <li key={`${i.id}:${i.version}`}><strong>{formatDateOnly(i.occurredOn)} {i.occurredTime}</strong> — <Badge tone={'neutral'}>{t(`safety.${i.status}`)}</Badge><br />{t('safety.description')}: {i.description}<br />{t('safety.actionTaken')}: {i.actionTaken}<br />{i.guardianInformed ? `${t('safety.guardianInformed')} — ${t(`safety.${i.contactMethod!}`)}` : t('safety.notInformed')}{i.followUp && <><br />{t('safety.followUp')}: {i.followUp}</>}{editor?.(i)}</li>)}</ul>;
}
function InformedFields({ informed,setInformed,method,setMethod,followUp,setFollowUp }: { informed: boolean; setInformed: (v: boolean) => void; method: string; setMethod: (v: string) => void; followUp: string; setFollowUp: (v: string) => void }) {
  const { t } = useLocale(); return <>
    <label className="choice-field"><input type="checkbox" checked={informed} onChange={(e) => setInformed(e.target.checked)} />{t('safety.guardianInformed')}</label>
...
function IncidentForm({ childId,save,busy }: { childId: string; save: Save; busy: boolean }) {
  ...
  return <form className="form-stack" onSubmit={(e) => { void submit(e); }}><fieldset disabled={busy} className="organization-fields"><legend>{t('safety.report')}</legend><p>{t('safety.incidentHelp')}</p>
...
function StaffSafety({ childId }: { childId: string }) {
  ...
  if (state.error) return <p className="inline-notice inline-notice--danger" role="alert">{t(state.error)}</p>;
  const view = state.data; if (!view) return <Skeleton label={t('state.loading')} />;
  const can = (c: string) => view.capabilities.includes(c); const active = view.child.status === 'ACTIVE';
  return <section className="workspace-section" aria-label={t('safety.title')}><Messages error={error} notice={notice} />
    {view.health && <Card title={t('safety.emergency')}><p>{t('safety.emergencyHelp')}</p>{!view.modules.health && <p className="inline-notice" role="status">{t('safety.healthDisabled')}</p>}<GuardianContacts guardians={view.guardians} />
    ...
    {view.pickup && <Card title={t('safety.pickup')}>{!view.modules.pickup && <p className="inline-notice" role="status">{t('safety.pickupDisabled')}</p>}
      <h3>{t('safety.authorized')}</h3>{view.pickup.authorizations.length ? <ul className="record-list">{view.pickup.authorizations.map((a) => <li key={`${a.id}:${a.version}`}>{a.fullName} ({a.relationship}) — <span dir="ltr">{a.mobile}</span> — <Validity from={a.validFrom} until={a.validUntil} /> — {a.blockers.length ? a.blockers.map((b) => t(`safety.blocker.${b}`)).join(', ') : t('safety.permitted')}
        {a.review && <> — {t(`safety.${a.review.decision}`)}</>}
        {can('pickup.manage') && view.modules.pickup && a.active && <> <Button icon="trash" variant="danger" disabled={busy} onClick={() => { void save(`pickup-authorizations/${a.id}/deactivate`,{ expectedVersion: a.version }); }}>{t('safety.deactivate')}</Button><ReviewForm authorization={a} save={save} busy={busy} /></>}</li>)}</ul> : <p>{t('safety.none')}</p>}
      {can('pickup.manage') && view.modules.pickup && active && <PersonForm childId={view.child.id} save={save} busy={busy} guardian={false} />}
      <h3>{t('safety.restrictions')}</h3>{view.pickup.restrictions.length ? <ul className="record-list">{view.pickup.restrictions.map((r) => <li key={`${r.id}:${r.version}`}>{t(`safety.${r.kind}`)}{r.fullName && ` — ${r.fullName}`}{r.mobile && <> — <span dir="ltr">{r.mobile}</span></>} — {r.summary}{r.privateNote && <> — <em>{r.privateNote}</em></>}{!r.active && <> ({t('safety.ended')})</>}
        {can('pickup.manage') && view.modules.pickup && r.active && <> <Button icon="trash" variant="danger" disabled={busy} onClick={() => { void save(`pickup-restrictions/${r.id}/deactivate`,{ expectedVersion: r.version }); }}>{t('safety.endRestriction')}</Button></>}</li>)}</ul> : <p>{t('safety.none')}</p>}
    ...
    {view.incidents && <Card title={t('safety.incidents')}>{!view.modules.incidents && <p className="inline-notice" role="status">{t('safety.incidentsDisabled')}</p>}
...
export function GuardianSafetyPanel({ childId }: { childId: string }) {
  ...
  return <section className="workspace-section" aria-label={t('safety.title')}><Messages error={error} notice={notice} />
    ...
    {view.pickup && <div id="pickup"><Card title={t('safety.pickup')}><h3>{t('safety.authorized')}</h3>{view.pickup.authorizations.length ? <ul className="record-list">{view.pickup.authorizations.map((a) => <li key={`${a.id}:${a.version}`}>{a.fullName} ({a.relationship}) — <span dir="ltr">{a.mobile}</span> — <Validity from={a.validFrom} until={a.validUntil} />{a.pendingNurseryConfirmation && <> — {t('safety.pending')}</>}
      {' '}<Button icon="trash" variant="danger" disabled={busy} onClick={() => { void save(`pickup-authorizations/${a.id}/deactivate`,{ expectedVersion: a.version }); }}>{t('safety.deactivate')}</Button></li>)}</ul> : <p>{t('safety.none')}</p>}
