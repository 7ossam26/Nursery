...
export class AuthClient {
  ...
  private async request(path: string, method = 'GET', body?: unknown): Promise<unknown> {
    const response = await this.transport(`/api/v1/auth/${path}`, { method, credentials: 'same-origin', cache: 'no-store', headers: method === 'GET' ? {} : { 'Content-Type': 'application/json', 'X-CSRF-Token': this.csrf }, ...(body !== undefined ? { body: JSON.stringify(body) } : {}) });
    if (!response.ok) {
