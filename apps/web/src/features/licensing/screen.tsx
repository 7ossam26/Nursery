import { PageHeader , Badge } from '../../components/data-display.js';
...
import { Button, DateField, SelectField, TextField, type ButtonVariant } from '../../components/controls.js';
...
function ReasonActionById({ label, run, extra, variant = 'primary' }: Readonly<{ variant?: ButtonVariant; label: string; run: (id: string, reason: string, extra: Record<string, string>) => Promise<void>; extra?: ReadonlyArray<{ key: string; label: string }> }>) {
  const { t } = useLocale();
  ...
  return <form className="form-stack" onSubmit={async (e) => {
    ...
    {error && <p className="inline-notice inline-notice--danger" role="alert">{t(error)}</p>}{done && <p className="inline-notice" role="status">{t('licensing.actionDone')}</p>}
    ...
    <Button variant={variant} loading={busy} type="submit" disabled={busy || !id.trim() || !reason.trim()}>{label}</Button>
  </form>;
}
...
function RenewalPaymentsCard({ payments, onRecorded }: Readonly<{ payments: LicenseContext['renewalPayments']; onRecorded: () => Promise<void> }>) {
  ...
  return <Card title={t('licensing.renewalPayments')}>
    <ul className="record-list">{payments.map((payment) => <li key={payment.id}>{payment.periodStart} → {payment.periodEnd}: {egp(payment.amountPiastres)} ({payment.method})</li>)}</ul>
    <form className="form-stack" aria-busy={busy} onSubmit={async (e) => {
...
export function LicensingScreen() {
  ...
  return <main className="organization-page"><PageHeader title={t('licensing.title')} icon="settings" />
    {error && <p className="inline-notice inline-notice--danger" role="alert">{t(error)}</p>}{notice && <p className="inline-notice" role="status">{notice}</p>}
    {!context ? <Skeleton label={t('state.loading')} /> : <>
      <Card title={t('licensing.status')}>
        <p><Badge tone={context.status==='ACTIVE'?'success':context.status==='GRACE'?'warning':'neutral'}>{t(`licensing.status.${context.status}`)}</Badge></p>
      ...
      {can('licensing.manage') && form && <Card title={t('licensing.capacities')}>
        <form className="form-stack" onSubmit={saveLimits} aria-busy={busy}>
          ...
          <Button loading={busy} icon="check" type="submit" disabled={busy}>{t('licensing.save')}</Button>
        </form>
      </Card>}
      {can('licensing.manage') && <RenewalPaymentsCard payments={context.renewalPayments} onRecorded={load} />}
      {can('seats.release') && <>
        <Card title={t('licensing.release')}><ReasonActionById variant="danger" label={t('licensing.release')} run={async (accountId, reason) => { await auth.client.licensing(`accounts/${accountId}/release`, 'POST', { reason }); await load(); }} /></Card>
        <Card title={t('licensing.restore')}><ReasonActionById variant="success" label={t('licensing.restore')} run={async (accountId, reason) => { await auth.client.licensing(`accounts/${accountId}/restore`, 'POST', { reason }); await load(); }} /></Card>
      </>}
      {can('support.access') && <Card title={t('licensing.supportContext')}>
        ...
        {supportError && <p className="inline-notice inline-notice--danger" role="alert">{t(supportError)}</p>}
        ...
        {supportResult && <div role="status">
          <p><Badge tone={'neutral'}>{t(`licensing.status.${supportResult.status}` as MessageKey)}</Badge></p>
...
function ModulesCard() {
  ...
  return <Card title={t('licensing.modules')}>
    <p>{t('licensing.modulesHelp')}</p>{error && <p className="inline-notice inline-notice--danger" role="alert">{t(error)}</p>}
    {modules?.map((module) => {
      ...
      return <fieldset key={module.moduleKey} className="organization-fields">
        ...
        {impact && <div>
          <ul className="record-list">{impact.impacts.map((line) => <li key={line}>{line}</li>)}</ul>
          {impact.requiresCatchupAcknowledgement && <>
            ...
            <Link className="button button--outline" to="/administration/billing">{t('billing.title')}</Link>
            <label className="choice-field"><input type="checkbox" checked={ack[module.moduleKey] ?? false} onChange={(e) => setAck((a) => ({ ...a, [module.moduleKey]: e.target.checked }))} />{t('licensing.catchupAcknowledge')}</label>
          </>}
          ...
          <Button loading={busy === module.moduleKey} icon="check" variant="secondary" disabled={busy === module.moduleKey || !(reason[module.moduleKey] ?? '').trim() || (impact.requiresCatchupAcknowledgement && !ack[module.moduleKey])} onClick={async () => {
...
function BrandingCard() {
  ...
  return <Card title={t('licensing.branding')}>
    {error && <p className="inline-notice inline-notice--danger" role="alert">{t(error)}</p>}{notice && <p className="inline-notice" role="status">{notice}</p>}
    {form && <form className="form-stack" aria-busy={busy} onSubmit={async (e) => {
      ...
      {failures.length > 0 ? <p className="inline-notice inline-notice--danger" role="alert">{t('licensing.contrastFail', { pairs: failures.map((f) => `${f.pair} ${f.ratio.toFixed(2)}:1`).join(', ') })}</p> : <p className="inline-notice" role="status">{t('licensing.contrastOk')}</p>}
      <Button loading={busy} icon="check" type="submit" disabled={busy || failures.length > 0}>{t('licensing.save')}</Button>
    </form>}
  </Card>;
}
...
export function SettingsScreen() {
  const auth = useAuth(); const { t } = useLocale();
  const can = (key: string) => auth.session?.account.capabilities.includes(key) ?? false;
  return <main className="organization-page"><PageHeader title={t('licensing.settingsTitle')} icon="settings" />
    ...
    {can('users.manage_staff') && <>
      ...
      <Card title={t('licensing.deactivate')}><ReasonActionById variant="danger" label={t('licensing.deactivate')} run={async (id, reason) => { await auth.client.licensing(`accounts/${id}/deactivate`, 'POST', { reason }); }} /></Card>
      <Card title={t('licensing.reactivate')}><ReasonActionById variant="success" label={t('licensing.reactivate')} run={async (id, reason) => { await auth.client.licensing(`accounts/${id}/reactivate`, 'POST', { reason }); }} /></Card>
    </>}
    {can('users.create_parent') && <ProvisionCard title={t('licensing.provisionParent')} action="parents" />}
    {can('parents.block') && <>
      <Card title={t('licensing.block')}>
        <ReasonActionById variant="danger" label={t('licensing.block')} extra={[{ key: 'publicMessage', label: t('licensing.publicMessage') }, { key: 'untilDate', label: t('licensing.untilDate') }]}
      ...
      <Card title={t('licensing.unblock')}><ReasonActionById variant="success" label={t('licensing.unblock')} run={async (id, reason) => { await auth.client.licensing(`accounts/${id}/unblock`, 'POST', { reason }); }} /></Card>
    </>}
  </main>;
}
