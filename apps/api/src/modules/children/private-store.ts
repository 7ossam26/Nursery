...
export class PrivateDocumentStore {
 constructor(readonly configuredRoot:string,readonly namespace:'child-documents'|'expense-documents'|'report-exports',readonly maxBytes=MAX_DOCUMENT_BYTES) {}
 async root() {
 ...
 async write(key:string,bytes:Buffer) {
  if(bytes.length>this.maxBytes)throw new Error('Private file exceeds size limit');
 ...
 async read(key:string,sha256:string) {
  ...
  if(!stat.isFile()||stat.isSymbolicLink()||stat.size>this.maxBytes) throw new Error('Invalid private file');
