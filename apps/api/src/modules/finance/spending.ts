...
export class SpendingService {
 ...
 async list(token:string,raw:unknown):Promise<ExpensePage> {
  ...
  return this.core.children.withPolicy(token,async(tx,p)=>{
   ...
   const totals=(await tx.query<{totalPending:string;totalPaid:string;totalCount:number}>(`select coalesce(sum(amount) filter(where state in ('PENDING','APPROVED')),0)::text as "totalPending",coalesce(sum(amount) filter(where state='PAID'),0)::text as "totalPaid",count(*)::int as "totalCount" from expense_states e where ${where}`,values)).rows[0];
   const items=(await tx.query<Expense>(`select id,branch_id as "branchId",classroom_id as "classroomId",category_id as "categoryId",category_name as "categoryName",amount::text,due_on::text as "dueOn",note,state,settlement_id as "settlementId",account_id as "accountId",method,paid_on::text as "paidOn" from expense_states e where ${where} order by due_on,id limit $8 offset $9`,[...values,q.limit,q.offset])).rows;return {...totals,items};
  });
 }
