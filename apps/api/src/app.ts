...
import { installImports } from './modules/imports/routes.js';
import { installSupport } from './modules/support/routes.js';
import { installStaticWeb } from './static.js';
...
export function buildApp(config: AppConfig, database: DatabaseLifecycle = createDatabase(config.databaseUrl)): FastifyInstance {
  ...
  const app = Fastify({ bodyLimit: 8192, trustProxy: config.trustProxy ?? false, logController: new LogController({ disableRequestLogging: true }), logger: { level: 'info', redact: ['req.headers.authorization', 'req.headers.cookie', 'req.headers.x-csrf-token', 'res.headers.set-cookie', 'sessionSecret', 'password', 'token'] }, genReqId: () => crypto.randomUUID() });
  ...
  if (config.webDistDir) installStaticWeb(app, config.webDistDir);
